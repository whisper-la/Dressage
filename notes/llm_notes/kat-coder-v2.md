...
2. **logprob 本身的噪声（报告的新发现）**：MoE 的随机路由、容量丢弃（capacity dropping：专家满了就把 token 丢给次选）、数值误差，使得**同一输入前向跑两次，logprob 都可能不一样**——logprob 是个随机变量。而重要性权重是 \(w = \exp(\text{logprob 之差})\)，**指数会把噪声放大**（见 §6.3 的数值例子）。
...
\[r_{turn}^{(n)}(\theta) = \prod_{i \in T_n} \frac{\pi_\theta(y_i \mid x, y_{<i})}{\pi_{\theta_{old}}(y_i \mid x, y_{<i})} \tag{6}\]

逐符号解释：
- \(\pi_\theta(y_i \mid x, y_{<i})\)：当前模型在"输入 \(x\) + 已生成前缀 \(y_{<i}\)"下生成 token \(y_i\) 的概率；分母是旧模型的同一概率；
