...
> **阅读路线**　本文以 DSpark 为终点与重点：第 1–2 章建立分析框架（瓶颈、无损验证、期望接受长度），第 3–6 章逐个拆解前代方案——每章的"局限"小节实则是一份**需求清单**，逐条对应 DSpark 的设计动机。时间紧的读者可按 1 → 6 → 7 跳读，再回头补细节。
...
**组件清单**。一轮解码由 5 个组件协作完成（目标模型全程冻结，只在最后出场一次）：

| 组件 | 一句话职责 | 成本画像 |
| --- | --- | --- |
| 并行 Backbone（DFlash 变体） | 一次前向算出整块草稿的表征与基础 logits | 重，但一轮只跑一次 |
| 串行头（Markov / RNN） | 逐位置注入"前一个草稿 token 是谁"，修多峰碰撞 | 极轻，但须串行走 $\gamma$ 步 |
| 置信度头 | 自报每个草稿位置的存活概率 | 一个线性层，可忽略 |
| 硬件感知前缀调度器 | 跨请求全局分配验证预算，砍低存活率后缀 | CPU 上排序 + 查表，可忽略 |
| 目标模型（冻结） | 一次前向验证：接受 / 修正 / 补 bonus token | 重，每轮固定一次 |

**名词约定**：图中的 "Backbone" 指**草稿器自己的主干**（数层 Transformer + KV 注入 + 块内双向注意力，参数量约为目标模型的 1–3%），**不是**目标模型的主干——这是读这张图最容易混淆的一点。

**一轮解码的总流程**（设草稿块长 $\gamma$）：

1. **定锚**：上一轮验证确认的末位 token $x_0$ 作为本轮 anchor；
2. **并行起草**：backbone 的输入是 $[\text{Emb}(x_0)$ 加 $\gamma-1$ 个可学习 mask 占位符$]$，块内双向注意力、每层吃目标模型注入的 KV 上下文，**一次前向**产出 $\gamma$ 个位置的隐藏状态 $\mathbf{h}_k$ 与基础 logits $U_k$——注意此刻各位置是独立猜的，彼此不知道对方会是什么 token；
3. **串行修正**：串行头从左到右走 $\gamma$ 步，第 $k$ 步把刚采出的 $x_{k-1}$ 经低秩转移矩阵变成词表偏置 $B(x_{k-1}, \cdot)$，与 $U_k$ 相加后采样出 $x_k$——块内依赖由此补上；
4. **自评**：与采样同步，置信度头输出逐位置存活条件概率 $c_k$，连乘得前缀存活率 $a_{\le k}$（须经 STS 校准，7.4 节）；
5. **调度**：调度器汇总**全体活跃请求**的 $\{a_{r,j}\}$ 与引擎实测步速曲线 $\mathrm{SPS}(B)$，按存活率全局降序贪心采纳，系统吞吐 $\Theta = \tau^* \cdot \mathrm{SPS}(B)$ 一下降就停，得到每请求的验证长度 $\ell_r^*$；
6. **验证与滚动**：目标模型一次前向验证每请求的前 $\ell_r^*$ 个草稿——逐位接受、第一个分歧点修正、再补一个 bonus token；被确认的末位 token 成为下轮 anchor，回到第 1 步。

下面按组件逐一解释（公式细节见 7.3–7.5 节）。

**组件一：并行 Backbone（图中的 PB → HID / LOG）**。

它是**草稿模型的主体**——完整地说，草稿模型 = 并行 backbone + 串行头 + 置信度头三件套，全部参数也只有目标模型的 1–3%，backbone 占其中 99% 以上的计算，故名"主干"。

- **输入**：$[\text{Emb}(x_0)$ 加 $\gamma-1$ 个可学习 mask 占位符$]$，外加 KV 注入的目标模型上下文；
- **输出**：每个位置的隐藏状态 $\mathbf{h}_k$（走上路 HID，喂两个小头）与基础 logits $U_k$（走下路 LOG，喂串行头）；
- **内部结构**：数层 Transformer，两处改造——①块内**双向注意力**：$\gamma$ 个位置互相可见（敢去掉因果约束，是因为 mask 位置是占位符、**没有真实未来内容可泄漏**，见 6.1 节）；②**KV 注入**：每层注意力的 K/V 前段拼接目标模型隐藏状态投影出的上下文，草稿时一直在"参考大模型的内部想法"而非盲猜；
- **与目标模型的纽带**：embedding 与 LM Head 直接共享目标模型的（冻结）——它与大模型说同一套词表；
- **成本画像**：草稿延迟的主体，"贵的部分保持并行"里那个贵的部分；一轮只跑一次。

**组件二：串行头（图中的 SH → SAMP）**。

*为什么需要它*：$U_k$ 是各位置独立猜的裸 logits。多峰上下文（"of course" 与 "no problem" 都合理）里，每个位置的边缘分布各自混合了两个模式，独立采样会拼出 "of problem" 这种跨模式垃圾——**多峰碰撞**。碰撞不是猜得不准，而是各位置的**模式选择没有对齐**。

*单步输入输出*：

| | 内容 | 来源 |
| --- | --- | --- |
| 输入 ① | $U_k$：位置 $k$ 的裸 logits（$V$ 维实数向量，"还没做决定的打分表"） | backbone |
| 输入 ② | $x_{k-1}$：上一步采出的 token（一个离散 id，"已经落子的决定"） | 自己上一步的采样（$k=1$ 时是 anchor $x_0$） |
| 输出 | 修正分布 $p_k$，从中采出 $x_k$ | — |

*内部三个动作*（以 $x_1$ = "of" 已采出、修位置 2 为例）：

1. **查表**：在 $\mathbf{W}_1$（$V \times 256$，词表每个 token 一行）里取出 "of" 那一行——256 维的"转移指纹"，编码"of 后面通常接什么"；
2. **投影**：乘以 $\mathbf{W}_2$（$256 \times V$）展开成词表大小的偏置 $B(\text{of}, \cdot)$——"course" 位 $+2.5$、"problem" 位 $-1.8$、绝大多数位置 ≈ 0；
3. **叠加采样**：$p_2 = \text{softmax}(U_2 + B)$——"course" 从 1.2 抬到 3.7、"problem" 从 1.4 压到 $-0.4$，正确模式压倒性胜出；采出 $x_2$ 传给下一步。

*为什么这样就能修多峰碰撞*：并行起草算的是**边缘分布的独立乘积** $p(x_1)\,p(x_2)$——四种组合各分一杯羹（"of course" / "no problem" / "of problem" / "no course" 各 0.25）；而**条件分布通常是单峰的**：$p(x_2 \mid x_1{=}\text{of})$ 几乎全压 "course"。串行修正把"边缘 × 边缘"换成"边缘 × 条件"，跨模式组合的概率归零——**模式只选一次（第一个位置），全队跟上**。

*为什么一张低秩表就够*：解锁模式选择的钥匙往往就在紧邻的前一个 token 里（看到 "of" 便知接 "course"），这种局部接续规律高度冗余，256 维低秩因子足以覆盖大部分碰撞（完整 $V \times V$ 转移矩阵需约 $10^{10}$ 参数，低秩后仅 $2Vr \approx 8\times 10^7$）。更长程的块内依赖是 RNN 版（GRU 门控状态 $\mathbf{s}_k$）的用武之地，但实测收益有限，生产默认 Markov 版。

*为什么串行却不慢*：它每步算的东西和自回归草稿器完全不同——

| | EAGLE（自回归草稿） | DSpark 串行头 |
| --- | --- | --- |
| 每步算什么 | 一层完整 Transformer 前向（注意力 + FFN） | 一次查表 + 一次 256 维小投影 |
| $\gamma$ 步总成本 | 毫秒级 × $\gamma$ | 微秒级 × $\gamma$，可忽略 |

**组件三：置信度头（图中的 CH）**。

- **输入**：$[\mathbf{h}_k;\ \mathbf{W}_1[x_{k-1}]]$——"这个位置算出了什么"（backbone 表征）加"前一个位置采了什么"（与串行头共享同一张 $\mathbf{W}_1$ 查找表，所以图里 `SAMP --> CH` 有条边）；
- **输出**：$c_k = \sigma(\mathbf{w}^\top[\cdot]) \in (0,1)$——"前面全被接受的前提下，本位置也被接受"的条件概率；
- **机制**：一个线性层 + sigmoid，成本可忽略；逐位置 $c_k$ 连乘得前缀存活率 $a_{\le k} = \prod_{i \le k} c_i$；
- **为什么必须校准**：调度器把连乘值当"该位置值多少钱"的估价参与全局优化，用的是**绝对数值**而非排名；神经网络的置信度天然过自信，连乘会把误差指数放大（未校准 $0.90 \times 0.85 \times 0.80 = 0.61$ vs 真实 $0.26$，调度结论会从"剪枝"反转成"扩验"）——STS 逐位置温度校准（7.4 节）就是为此；
- **训练信号**：无需人工标注，软标签 $c_k^* = 1 - \frac{1}{2}\|p_k^d - p_k^t\|_1$（草稿与目标分布的总变差距离，7.5 节）。

**组件四：硬件感知前缀调度器（图中的 SCH）**。

- **输入**：**全体活跃请求**的存活率 $\{a_{r,j}\}$ + 引擎实测步速曲线 $\mathrm{SPS}(B)$（当前负载）；
- **输出**：每请求的验证长度 $\ell_r^*$；
- **机制**：所有候选位置 $(r, j)$ 按存活率全局降序贪心采纳，每采纳一个就更新系统吞吐 $\Theta = \tau^* \cdot \mathrm{SPS}(B)$，$\Theta$ 一下降立即停（贪心合法性与无损性论证见 7.4 节）；
- **为什么必须全局且感知负载**：高并发下每多验一个 token 就占一个 batch 槽位（系统吞吐本身，6.4 节）——同一个模型在低负载时应多验（白赚 token）、高负载时应砍低置信后缀。静态阈值策略做不到这一点，因为它不看负载；
- **在图中的位置**：`SCH -->|保留前缀 ℓ*| VER` 是全图唯一一条"砍数据"的边——被砍的后缀**根本不会进入目标模型**。

**组件五：目标模型验证与回环（图中的 VER → A）**。

- **输入**：每请求前 $\ell_r^*$ 个草稿 token；
- **输出**：接受的最长前缀 + 分歧点修正 token + 一个 bonus token；
- **机制**：一次 teacher forcing 前向——对**已知**序列打分是自回归模型天然可并行的操作（训练时就是这么算的），所以 $\ell^*$ 个位置一次算完；按拒绝采样从位置 1 起逐位接受，第一个分歧点用目标分布修正、其后作废（2.3–2.4 节，输出与目标模型严格同分布）；
- **回环**：末位确认 token 成为新 anchor（`VER --> A`），下一轮从这里重新出发；若本轮草稿全被接受，一轮净落袋 $\ell^* + 1$ 个 token；
- **成本画像**：目标模型每轮只出场这一次——全部组件的存在，都是为了让这一次前向"值回票价"（接受尽量长）且"不占多余槽位"（不验必死的后缀）。

**时间轴感受**（单请求视角）：PB 一次前向（草稿延迟主体）→ SH/SAMP 走 $\gamma$ 步（每步微秒级查表）→ CH 与采样同步完成 → SCH 在 CPU 上瞬间出结果 → VER 一次前向。总延迟由 PB 与 VER 两个前向主导，中间三个组件合计不到零头——这就是"贵的部分保持并行、依赖建模只交给极轻串行模块"的图形化表达。
...
**并行级的内部结构图**（草稿模型的主体，一个独立的小 Transformer）：

```mermaid
flowchart TB
    subgraph IN["输入组装（γ 个位置）"]
        X0["Emb(x₀)<br/>共享 embedding（冻结）"]
        MK["γ−1 个可学习 mask 向量<br/>（占位符，无真实内容）"]
    end
    CTX["目标模型多层隐藏状态<br/>拼接 → W_c 投影 → RMSNorm<br/>得上下文特征 H_ctx（6.1 节）"]
    subgraph LAY["L 层 Transformer（生产：3 个 MoE 层）"]
        ATTN["每层：块内双向注意力<br/>K/V = [H_ctx ; 块内位置]"] --> FFN["FFN / MoE + 残差"]
    end
    X0 --> LAY
    MK --> LAY
    CTX -->|"注入每层注意力的 K/V 前段"| ATTN
    LAY --> HID["隐藏状态 h₁…h_γ"]
    HID --> LM["共享 LM Head（冻结）"] --> U["基础 logits U₁…U_γ"]
    HID -->|表征通路| HEADS["串行头 / 置信度头"]
    U -->|logits 通路| HEADS
```
...
```mermaid
flowchart LR
    X["xₖ₋₁（上一步采出的 token id）"] --> LUT["W₁ 查表<br/>取第 xₖ₋₁ 行<br/>→ 256 维转移指纹"]
    LUT --> PROJ["× W₂ 投影<br/>→ V 维偏置 B(xₖ₋₁, ·)"]
    U["Uₖ（backbone 裸 logits）"] --> ADD["逐位相加"]
    PROJ --> ADD
    ADD --> SM["softmax → 修正分布 pₖ"]
    SM --> SAMP["采样得 xₖ"]
    SAMP -->|"作为下一步输入"| LUT
```

读法：没有注意力、没有 FFN，整条链只有"查一行 → 投影 → 加分 → 采样"四个动作；唯一的回路是底部那条反馈边——采出的 $x_k$ 变成下一步的查表索引。**依赖链很短（每步微秒级），但每步都携带了上一步的选择**——这就是"串行"的全部含义与全部代价。
...
**RNN head 的单步数据流图**（与 Markov head 对照看：多了一路状态记忆与 backbone 表征输入）：

```mermaid
flowchart LR
    S["sₖ₋₁（上一步循环状态）"] --> Z["拼接 zₖ ∈ R^{2r+d}"]
    X["W₁[xₖ₋₁]（前 token 指纹）"] --> Z
    H["hₖ（backbone 当前位置表征）"] --> Z
    Z --> G["GRU 式门控更新<br/>遗忘门 σ(W_g zₖ) ⊙ sₖ₋₁<br/>+ 写入 (1−门) ⊙ tanh(W_c zₖ)"]
    G --> B["Bₖ = W₂ᵀ tanh(W_o zₖ)<br/>→ 加进 Uₖ 后采样"]
    G -->|"sₖ 传给下一步"| S
```

读法：Markov head 的"记忆"只有一步（$x_{k-1}$ 本身），RNN head 把记忆换成可学习的循环状态 $\mathbf{s}_k$——门控决定每一步忘掉多少旧状态、写入多少新信息，理论上能记住块内全部已采前缀；同时它还多吃一路 $\mathbf{h}_k$（backbone 对该位置的表征），即"参考当前位置的语义"而不仅是"参考前一个 token"。代价是每步从两次小矩阵乘升级为若干次 $r \times (2r+d)$ 投影，且状态链路更长。
...
**置信度头的数据流图**（从单位置打分到调度器可用的存活率）：

```mermaid
flowchart LR
    H["hₖ（backbone 表征）"] --> CAT["拼接"]
    E["W₁[xₖ₋₁]（前 token 指纹）<br/>与串行头共享查找表"] --> CAT
    CAT --> LIN["线性层 w"] --> SIG["sigmoid"] --> C["cₖ ∈ (0,1)<br/>条件接受概率"]
    C --> MUL["逐位置连乘<br/>a≤k = ∏ᵢ cᵢ"]
    MUL --> STS["STS 序贯温度校准<br/>（留出集离线拟合）"]
    STS --> OUT["校准后的存活率 a_{r,j}<br/>→ 交给调度器"]
```

读法：头部本身（拼接 → 线性 → sigmoid）平凡到不能再平凡，**这个组件的真正内容在图的后半截**——连乘把逐位置概率折算成前缀存活率（"第 $k$ 位值多少钱"的估价），STS 校准保证这个估价的绝对数值可信。两个输入各管一件事：$\mathbf{h}_k$ 回答"这个位置算出了什么"（内容置信度），$\mathbf{W}_1[x_{k-1}]$ 回答"前一个位置选了谁"（衔接置信度）——两个容易踩坑的位置（难内容、难衔接）都能被识别。
...
**调度器的内部结构图**（一个纯 CPU 的小模块：排序 + 贪心循环 + 查表）：

```mermaid
flowchart TB
    subgraph INP["两路输入"]
        C["全体活跃请求的逐位置置信度 c_{r,j}<br/>（来自各请求的置信度头，经 STS 校准）"]
        SP["离线实测的引擎步速曲线 SPS(B)<br/>（当前负载的定价表）"]
    end
    C --> MUL["连乘：前缀存活率 a_{r,j} = ∏ᵢ c_{r,i}"]
    MUL --> SORT["构造全局候选池并按 a 降序排序"]
    SORT --> LOOP["贪心循环：采纳队首 (r,j)<br/>ℓ_r ← j，B ← B+1，τ* ← τ*+a_{r,j}"]
    SP --> LOOP
    LOOP --> CALC["查表更新 Θ = τ* × SPS(B)"]
    CALC --> DEC{"Θ 仍在上升？"}
    DEC -->|"是：继续采纳"| LOOP
    DEC -->|"否：早停"| OUT["输出每请求验证长度 ℓ*₁…ℓ*_R<br/>→ 交给目标模型验证"]
```

读法：它是一个**估价-定价-拍卖**的三段结构——置信度头提供"估价"（这个草稿 token 多大概率能活），$\mathrm{SPS}(B)$ 曲线提供"定价"（当前负载下一个 batch 槽位值多少吞吐），贪心循环是"拍卖"（预算全局流通，价高者得验证名额，直到再买一个就亏）。早停既是效率优化（不用遍历全池），也是无损性约束的实现（不偷看未来 token，见下文因果约束段）。
...
**验证器（最后一个组件，机制承接第 2 章）**。调度器裁完之后，目标模型对每个请求的前 $\ell_r^*$ 个草稿做裁决：

```mermaid
flowchart LR
    D["各请求的草稿前缀<br/>x₁…x_ℓ*"] --> TF["目标模型 teacher forcing 前向<br/>一次算出各位置目标分布 p*_k"]
    TF --> CMP["逐位置拒绝采样比较"]
    CMP --> ACC["位置 1…j：接受"]
    CMP --> FIX["首个分歧点 j+1：<br/>从修正分布重采"]
    ACC --> BON["末尾白送一个 bonus token<br/>（p*_{ℓ*+1} 本来就算出来了）"]
    FIX --> BON
    BON --> N["末位确认 token<br/>→ 下一轮的 anchor"]
```

读法：验证器本身**没有任何新机制**——2.3 节的拒绝采样、2.4 节的无损性证明原样生效；DSpark 对它的全部改造发生在上游（调度器决定喂给它多长）。注意 bonus token 是免费的：teacher forcing 前向本来就产出第 $\ell^*+1$ 位的目标分布，全接受时直接笑纳。
